 ...
 * Module:      Demodulator.java Description: This class implements demodulation of various analog radio modes (FM, AM, SSB). It runs as a separate thread. It will read raw complex
 * samples from a queue, process them (channel selection, filtering, demodulating) and forward the to an AudioSink thread.
 *
 * @author Dennis Mantz
 *         Copyright (C) 2014 Dennis Mantz License: http://www.gnu.org/licenses/gpl.html GPL version 2 or higher
 ...
 *         You should have received a copy of the GNU General Public License along with this library; if not, write to the Free Software Foundation, Inc., 51 Franklin Street, Fifth
...
public class Demodulator extends Thread
{
  ...
  private static final int[] QUADRATURE_RATE = {
      1,        // off; this value is not 0 to avoid divide by zero errors!
      2 * AUDIO_RATE,  // AM
      2 * AUDIO_RATE,  // nFM
      8 * AUDIO_RATE,  // wFM
      2 * AUDIO_RATE,  // LSB
  ...
  private static final int[] MIN_USER_FILTER_WIDTH = {
      ...
      1500,  // USB
      3000};  // ASK/OOK
  private static final int[] MAX_USER_FILTER_WIDTH = {
      0,    // off
      15000,  // AM
      15000,  // nFM
      120000,  // wFM
  ...
  private int highlen = 0; // number of high pulses in ASK/OOK
  private int lowlen = 0;  // number of low pulses for debouncing in ASK/OOK
  private double pl = 350e-6;                // pulse length
  private double lowdur;            // pulse duration for a 0
  private int highmult = 3;              // multiplier for a 1
  private double highdur; // pulse duration for a 1
  private double dbdur;           // debouncing duration
  ...
  /**
   * Starts the thread. This thread will start 2 more threads for decimation and audio output. These threads are managed by the Demodulator and terminated, when the Demodulator
   * thread terminates.
   */
  @Override
  ...
  public void run()
  {
    ...
    while (!stopRequested)
    {
      ...
      switch (demodulationMode)
      {
        ...
        case DEMODULATION_ASKOOK:
          demodulateASKOOK(quadratureSamples, audioBuffer);
          break;
  ...
  /**
   * Will filter the samples in input according to the user filter settings. Filtered samples are stored in output. Note: All samples in output will always be overwritten!
   *
  ...
  /**
   * Will FM demodulate the samples in input. Use ~75000 deviation for wide band FM and ~3000 deviation for narrow band FM. Demodulated samples are stored in the real array of
   * output. Note: All samples in output will always be overwritten!
   *
  ...
  /**
   * Will AM demodulate the samples in input. Demodulated samples are stored in the real array of output. Note: All samples in output will always be overwritten!
   *
   * @param input incoming (modulated) samples
   * @param output outgoing (demodulated) samples
   */
  ...
  /**
   * Will SSB demodulate the samples in input. Demodulated samples are stored in the real array of output. Note: All samples in output will always be overwritten!
   *
   * @param input incoming (modulated) samples
   * @param output outgoing (demodulated) samples
   ...
   * @param output outgoing (demodulated) samples
   */
  private void demodulateASKOOK(SamplePacket input, SamplePacket output)
  {
    float[] reIn = input.re();
    float[] imIn = input.im();
    float[] reOut = output.re();
    float[] imOut = output.im();
    int inputSize = input.size();
    float quadratureGain = QUADRATURE_RATE[demodulationMode] / (2 * (float)Math.PI * 3000);
    ...
    for (int i = 0; i < output.size(); i++)
    {
      ...
      else
      {
        ...
        if (lowlen > dbdur && highlen > 0)
        {
          // if it's close enough to a low or high pulse, print it
          if (Math.abs(highlen - lowdur) < dbdur + 10)
          {
            b.append(0);
          }
          else if (Math.abs(highlen - highdur) < dbdur * highmult)
          {
            b.append(1);
          }
        }
      ...
      if (b.length() > 0)
      {
        Log.d(LOGTAG, "demodulateASKOOK: " + b.toString());
      }
    }
